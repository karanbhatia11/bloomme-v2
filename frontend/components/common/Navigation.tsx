...
export const Navigation: React.FC = () => {
  ...
  return (
    <nav className="fixed top-0 w-full z-50 glass-nav shadow-[0_20px_40px_rgba(47,21,0,0.06)]">
      <div className="flex justify-between items-center w-full px-6 md:px-12 py-3 max-w-[1440px] mx-auto">
        ...
        <div className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-sm font-medium text-on-background opacity-80 hover:text-primary transition-all"
            >
              {link.label}
            </Link>
          ))}
      ...
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -10 }}
          className="md:hidden bg-surface px-6 py-4 border-t border-outline-variant/10"
        >
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
